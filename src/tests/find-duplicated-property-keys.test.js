...
describe(`Validate a JSON file that does not contain any duplicates`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file.json`)))
...
describe(`String property <instance>.name is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string.json`)))
...
describe(`String property <instance>.name occurs three times`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_triplicated_string.json`)))
...
describe(`Integer property <instance>.year is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_integer.json`)))
...
describe(`Number property <instance>.weight is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_number.json`)))
...
describe(`Boolean property <instance>.isBoolean is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_boolean.json`)))
...
describe(`Array of string property <instance>.pets is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_array_of_string.json`)))
...
describe(`Array of integer property <instance>.good_years is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_array_of_integer.json`)))
...
describe(`Object property <instance>.myObject is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_object.json`)))
...
describe(`Property <instance>.year is duplicated and one property contains an integer whereas the other one contains a string value`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_property_with_different_value_types.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `year`], 2)])
  })
})
...
describe(`Property <instance>.name is duplicated and contains the same values for each property`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_with_same_values.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `name`], 2)])
  })
})
...
describe(`Object property <instance>.myObject is duplicated and both properties have the same content`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_object_with_same_content.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `myObject`], 2)])
  })
})
...
describe(`Object property <instance>.myObject is duplicated and one property also has a duplicated boolean property`, () => {
  it(`returns the two expected property objects`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_object_with_duplication_in_one_object.json`)))
...
describe(`Object property <instance>.myObject is duplicated and both properties also have a duplicated boolean property`, () => {
  it(`returns the three expected property objects`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_object_with_duplication_in_both_objects.json`)))
...
describe(`One string property is contained by an object Object which is also contained by another object`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_deep_within_an_object.json`)))
...
describe(`One array of string porperty is contained by an object Object which is also contained by another object`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_array_deep_within_an_object.json`)))
...
describe(`Root is duplicated`, () => {
  it(`does not validate anything but throws the expected exception as an invalid value has been passed as argument`, () => {
    const filePath = readFile(path.join(appRootPath, `./assets/test_files/root_is_duplicated.json`))
...
describe(`Root is a valid array`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/root_is_an_array.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Root is an array but one of the objects contained by the array contains a duplicated string property`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/root_is_an_array_and_one_contains_a_duplicated_string.json`)))
...
describe(`Root is an array but one of the objects contained by the array contains a duplicated string property whereas the other one contains a duplicated array of strings`, () => {
  it(`returns the two expected property objects`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/root_is_an_array_and_both_contain_a_duplicated_property.json`)))
...
describe(`String property <instance>.name is duplicated and contains a comma within its value`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_with_comma_value.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `name`], 2)])
  })
})
...
describe(`String property <instance>.name is duplicated and contains a comma within its key`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_with_comma_key.json`)))
...
describe(`String property <instance>.name is duplicated and contains a quote within its value`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_with_quote_value.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `name`], 2)])
  })
})
...
describe(`Validate a JSON file that has an array with duplicated string values`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_in_array.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that has an array with duplicated object values`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_object_in_array.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that has an array object with duplicated property keys`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_property_key_in_array_object.json`)))
...
describe(`Validate a JSON file that has an array whose second object contains a duplicated property key`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_property_key_in_second_array_object.json`)))
...
describe(`Validate a valid JSON file that containy an empty string property <instance>.`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_with_string_for_empty_property_key.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Empty string property <instance>. is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_for_empty_property_key.json`)))
...
describe(`String property <instance>.name\\ is duplicated`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_whose_key_ends_with_a_backslash.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `name\\\\`], 2)])
  })
})
...
describe(`Duplicated object property 'definition' exists in a deeply nested array`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_object_deeply_nested_within_an_array.json`)))
...
describe(`Duplicated string property 'name' exists in an array of object array`, () => {
  it(`returns the expected property object`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_property_in_array_of_object_array.json`)))
...
describe(`String property 'name' and integer property 'year' are duplicated`, () => {
  it(`returns the two expected property objects`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/duplicated_string_and_integer.json`)))
...
describe(`Validate a JSON file that does not contain any duplicates but contains an additional space character between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_space_between_key_and_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that does not contain any duplicates but contains an additional tab character between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_tab_between_key_and_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that does not contain any duplicates but contains three additional space characters between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_three_spaces_between_key_and_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that does not contain any duplicates but contains three additional tab characters between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_three_tabs_between_key_and_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that does not contain any duplicates but contains an additional space character followed by an additional tab character between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_space_and_tab_between_key_and_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that does not contain any duplicates but contains an additional tab character followed by an additional space character between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_tab_and_space_between_key_and_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that has a duplicated property <instance>.name but one key has an additional space character between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_containing_a_space_between_key_and_colon.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `name`], 2)])
  })
})
...
describe(`Validate a JSON file that has a duplicated property <instance>.name and property <instance>.weight has an additional tab character between its key and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/one_duplicated_string_and_a_tab_between_another_key_and_colon.json`)))
    comparePropertyKeyArrays(duplicatedProperties, [createPropertyKey([`<instance>`, `name`], 2)])
  })
})
...
describe(`Validate a JSON file that does not contain any duplicates but contains a new line between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_new_line_between_key_and_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
...
describe(`Validate a JSON file that does not contain any duplicates but contains a space character new line and tab characater between key <instance>.name and the subsequent colon`, () => {
  it(`returns an empty list`, () => {
    const duplicatedProperties = findDuplicatedPropertyKeys(readFile(path.join(appRootPath, `./assets/test_files/valid_JSON_file_containing_space_new_line_and_tab_before_colon.json`)))
    expect(duplicatedProperties).toHaveLength(0)
  })
})
